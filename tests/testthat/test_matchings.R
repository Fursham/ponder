...
test_that("Test the structure of outputs", {
  ...
  expect_equal(length(matched1), 52)
  expect_equal(length(matched2), 52)
  expect_equal(names(mcols(matched2)), c('type','transcript_id','gene_id',
                                         'old_gene_id', 'match_level','gene_name'))
})

test_that("Test the value of outputs", {
  
  expect_equal(seqlevels(matched1), 'chr3') 
  expect_equal(unique(mcols(matched2)$gene_id), 'ENSMUSG00000028134.11') 
