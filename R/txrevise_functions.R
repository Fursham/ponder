...
indentifyAddedRemovedRegions <- function(tx1_id, tx2_id, exons_list){
  ...
  shared_region = GenomicRanges::GRanges(seqnames = extractFeature(seqnames(shared_exons)),
                          IRanges::IRanges(min(start(shared_exons)),max(end(shared_exons))), 
                          strand = extractFeature(strand(shared_exons)))
  addedInSecond = GenomicRanges::setdiff(exon_set2, shared_exons)
  addedInSecond = compareGRanges(addedInSecond, shared_region)
  removedFromFirst = GenomicRanges::setdiff(exon_set1, shared_exons)
