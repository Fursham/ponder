...
run <- function(query,
  ...
  pregroup <- rep(1:10, length.out = nrow(report_df))
  report_df <- bind_cols(tibble(pregroup), report_df)
  
  for (i in 1:10) {
    message(i)
    thisgroup = report_df[report_df$pregroup == i]
    
    group <- rep(1:clusters, length.out = nrow(thisgroup))
    thisgroup <- bind_cols(tibble(group), thisgroup)
    cluster <- create_cluster(cores = clusters, quiet = TRUE)
    
    parallel_df = thisgroup %>% partition(group, cluster = cluster)
    parallel_df %>%
      # Assign libraries
